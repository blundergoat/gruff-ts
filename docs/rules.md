...
Every medium- and low-confidence rule carries reviewed false-positive guidance:
the code shapes that trip it without being the defect, and what to do about each
one. That guidance lives on the rule catalogue rather than in this list, so it
cannot drift from what the analyser actually ships. Read it per rule with
`gruff-ts list-rules <rule-id>`, or across the catalogue in the
`falsePositiveShapes` field of `--format=json`.
