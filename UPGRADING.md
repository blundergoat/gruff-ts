...
17. **`summary` exits 0 unless asked** — The default gate for `summary` moves from `advisory` to `none`. A CI job that relied on `gruff-ts summary` exiting `1` on findings must pass `--fail-on advisory` (or another severity) or set `failOn.summary` in `.gruff-ts.yaml`. A config generated by an earlier `init` already pins `summary: advisory` and keeps gating.
