...
run_full() {
  ...
  expect_block writes "echo msg | git commit -F -" "piped git commit (stdin message)"
  expect_block writes "cat msg.txt | git commit -F -" "cat-piped git commit"
  expect_block writes "echo y | git -C . commit --no-verify -m x" "piped git -C commit no-verify"
  expect_block writes "printf x | git reset --hard HEAD~1" "piped git reset hard"
  expect_block writes "true | git clean -fd" "piped git clean force"
  expect_allow writes "git log --oneline | grep commit" "git log piped to grep (commit is a grep arg, not a subcommand)"
