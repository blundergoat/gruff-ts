...
While relocating the baseline cluster (M01/M03/M24) from 0.3.0 into 0.4.0, the operator flagged that keying a persistent baseline on the line-bearing fingerprint is too fragile: any pure code move shifts line numbers, changes the fingerprint, and resurfaces an already-baselined finding as new. The directive was to key the on-disk baseline on file occurrence count instead of line, following the PHPStan baseline model.
...
The prior plan (M24) assumed the opposite - that "a line-moved entry still matches the same fingerprint". That assumption is false against the current `makeFinding` and is the latent bug this ADR closes (recorded as a footgun in `.goat-flow/learning-loop/footguns/schema-and-cli.md`, search: `fingerprint embeds`).
