...
# Dashboard footguns (`src/dashboard.ts:startDashboard`)
...
The `/` page and the `/scan` HTML response in `startDashboard` are built by string concatenation. `escapeHtml` is applied to user-controlled values (`projectRoot`, finding messages, file paths), but every NEW field added to the page MUST be wrapped in `escapeHtml` explicitly - there is no template-engine default. The same applies to `renderHtml` (`src/report-html.ts`, search: `function renderHtml`), which also owns `escapeHtml`. Forgetting `escapeHtml` on a new field is a stored-XSS-shaped bug visible only when the dashboard is rendered.
